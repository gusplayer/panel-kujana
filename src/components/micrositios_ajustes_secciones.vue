<template>
	<el-collapse v-model="activeName" accordion>
		<el-collapse-item title="Nombre de la seccion" name="0">
			<el-input placeholder="Nombre de la seccion" v-model="currentSeccion.titulo"></el-input>
			<el-button v-on:click="editarTitulo">Guardar</el-button>
		</el-collapse-item>
		<el-collapse-item title="Texto" name="1">
		...
		<el-collapse-item title="Videos" name="2">
			<div class="videos_item" v-for="(video, index) in currentSeccion.videos" :key="index">
				<a :href="video.video" target="_blank">
					<img :src="setImgOfVideo(video.video)">
				</a>
				<div class="videos_item_ajustes">
					<el-button type="danger" icon="delete" v-on:click="deleteVideo(video.id, index)">Eliminar</el-button>
				</div>
			</div>
			<el-input placeholder="Url de youtube" v-model="video"></el-input>
			<el-button type="primary" class="create_video" v-on:click="createVideo">Crear video</el-button>
		</el-collapse-item>
		...
		<el-collapse-item title="Galeria" name="4">
			<div class="galeria">
				...
				<div class="galeria_item" v-for="(imagen, index) in currentSeccion.imagenes" :key="index">
		...
		<el-collapse-item title="Urls externas" name="5">
			...
			<div class="nueva_url_externa">
				...
				<div class="row">
					...
					<el-button type="primary"><input type="file" class="url_choose_file" v-on:change="uploadPhotoUrlExterna"><i class="fa fa-camera-retro" style="cursor: pointer;" aria-hidden="true"></i> imagen</el-button>
				</div>
				<el-button-group>
...
export default {
	...
	methods: {
      ...
      axios.delete(`${this.$urlHttp}/api/secciones/iframes/${id}`).then((response) => {
		...
		reloadIframe(){
			document.getElementById('iframePreview').src = `https://meta.gov.co/micrositio/${this.currentSeccion.micrositio_id}`;
		}
	}
}
</script>
