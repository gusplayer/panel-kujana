    ...
    <el-dialog title="Editar carpeta" :size="sizeDialog" v-if="currentFolderEdit" :visible.sync="dialogEditFolder">
      ...
      <span slot="footer" class="dialog-footer">
        <el-popover
          ...
          placement="top"
          width="160"
          v-model="visibleDeleteFolder">
          <p>Estas seguro de eliminar la carpeta?</p>
          <div style="text-align: right; margin: 0">
            <el-button size="mini" type="text" @click="visibleDeleteFolder = false">cancel</el-button>
            <el-button type="primary" size="mini" v-on:click="deleteFolder">confirm</el-button>
          </div>
        </el-popover>
        <el-button type="danger" v-popover:deleteFolder icon="delete">Eliminar</el-button>
      ...
      <div id="columns" class="documents_columns">
        <div class="documents__column" v-for="(column, index) in columns">
          ...
          <div class="documents_actions">
            ...
            <label :for="`uploadFilesDocument${index}`" v-if="allow() && index != 0" type="button" class="documents__action uploadFile" v-on:click="beforeUploadFile(column, index)" ><i class="material-icons">cloud_upload</i>Subir Archivo<input :id="`uploadFilesDocument${index}`" type="file" v-on:change="uploadFile" multiple></label>
          </div>
        </div>
...
export default {
  ...
  methods: {
    sendOrder(index){
      ...
      axios.post(`${this.$urlHttp}/carpeta/ordenar`, json).then((response) => {})
    },
    checkMove(evt){
    ...
    getFiles(){
      ...
      axios.post(`${this.$urlHttp}/carpeta/ver`, json).then((response) => {
        let newColumn = response.data.data.subcarpetas.concat(response.data.data.archivos);
        newColumn = newColumn.sort((a, b) => a.orden - b.orden)
    ...
    getContentFile(){
      ...
      this.currentPassword = '';
      axios.post(`${this.$urlHttp}/archivo`, json, { responseType: 'arraybuffer' }).then((response) => {
        let image = btoa(
    ...
    deleteFolder(){
      axios.delete(`${this.$urlHttp}/carpetas/${this.currentFolderEdit.id}`).then((response) => {
    ...
    newFolder(){
      ...
      axios.post(`${this.$urlHttp}/carpetas`, json).then((response) => {
        this.newFolderName = '';
    ...
    uploadFile(file){
      for(var i = 0; i < file.target.files.length; i++) {
        ...
        if(isLt30M){
          ...
          axios.post(`${this.$urlHttp}/archivos`, newFile, config).then((response) => {
            this.columns[this.currentPushData].push(response.data.data);
          }).catch((error) =>{
    ...
    editFolder(){
      ...
      axios.post(`${this.$urlHttp}/carpetas/editar`, json).then((response) => {
        this.dialogEditFolder = false;
      })
    },
    dialogDelete(id){
    ...
    deleteFile(){
      axios.delete(`${this.$urlHttp}/archivos/${this.currentIdFileDelete}`).then((response) => {
