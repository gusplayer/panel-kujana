...
<script>
import AjustesEditar from './convocatorias_ajustes_editar.vue';
import Preview from './convocatorias_preview.vue';
...
export default {
  ...
  computed: {
    micrositioData(){
      return this.$store.state.micrositiosData.filter(micrositio => micrositio.id == 47)[0];
    }
  },
  methods: {
    getData(){
      this.loadData = false;
      axios.get(`${this.$urlHttp}/api/secciones/47`).then((response) => {
