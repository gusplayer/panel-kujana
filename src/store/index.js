...
export const store = new Vuex.Store({
  ...
  mutations: {
    ...
    users (state) {
      axios.get('https://intranet.meta.gov.co/users/get').then(response =>  {
        let newArray = [];
        for(let dependencia of response.data.dependencias){
          dependencia.funcionarios = response.data.dependencias_usuarios[dependencia.nombreDependencia];
    ...
    timeline (state) {
      axios.get('https://intranet.meta.gov.co/timeline').then(response =>  {
        state.timelineData = response.data.timeline;
        state.timelineDataTMP = response.data.timeline;
        state.circularData = response.data.timeline.filter((article) => article.idDependencia == 37);
        state.circularDataTMP = response.data.timeline.filter((article) => article.idDependencia == 37);
      })
    },
    SEARCH_ARTICLE (state, search) {
      state.timelineData = state.timelineDataTMP.filter(article => article.titulo.toLowerCase().includes(search.toLowerCase()));
    },
    SEARCH_CIRCULAR(state, search) {
      state.circularData = state.circularDataTMP.filter(article => article.titulo.toLowerCase().includes(search.toLowerCase()));
    },
    diary (state) {
      axios.get('https://intranet.meta.gov.co/agenda').then(response => {
    ...
    folders (state) {
      axios.get('https://intranet.meta.gov.co/carpetas').then(response => {
        state.foldersData = response.data.carpetas;
      })
    },
    micrositios (state) {
      axios.get('https://intranet.meta.gov.co/api/micrositio').then(response => {
